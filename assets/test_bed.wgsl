...
struct MeshVertexOutput {
    ...
    /// At the fragment shader input:
    /// position.xy: current fragment position located at half-pixel centers
    /// [0 .. render target viewport size] eg. [(0.0, 0.0) .. (1280.0, 720.0)]
    /// position.z: current fragment depth where 1.0 is near clipping plane, and 0.0 is inf far away
    ///     (near / in.position.z is the linear world space distance from camera)
    /// position.w: current fragment depth where 1.0 / position.w is the linear world space distance from camera
...
fn fragment(in: MeshVertexOutput) -> @location(0) vec4<f32> {
    ...
    let ndc_a = vt::frag_coord_to_ndc(in.position);
